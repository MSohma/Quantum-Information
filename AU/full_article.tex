...
$$
As stated in Sec. 3, after such preparation, we can define the modular operator $\Delta$ for the von Neumann algebra $\mathfrak{M}=\ell(\mathfrak{N})$ and its cyclic separating vector $\rho^{1/2}$. 
...
Remark that we have
$$
\mathfrak{B}(\mathfrak{H}_2)=v^\ast \mathfrak{B}({\cal H}\otimes{\cal H})v.
$$
